...
protected:
    OptionsTest() :
        ...
        default_redis_server_list_.push_back("127.0.0.1:6379");
    }
...
TEST_F(OptionsTest, NoArguments) {
    ...
    EXPECT_EQ(options_.cassandra_use_ssl(), false);
...
TEST_F(OptionsTest, DefaultConfFile) {
    ...
    char *argv[argc];
    char argv_0[] = "options_test";
    char argv_1[] = "--conf_file=src/contrail-analytics/contrail-query-engine/contrail-query-engine.conf";
    argv[0] = argv_0;
    argv[1] = argv_1;

    options_.Parse(evm_, argc, argv);
    vector<string> passed_conf_files;
    passed_conf_files.push_back("src/contrail-analytics/contrail-query-engine/contrail-query-engine.conf");
...
TEST_F(OptionsTest, CustomConfigFile) {
    ...
    redis_server_list.push_back("10.10.10.1:6379");
    redis_server_list.push_back("20.20.20.2:6379");
    redis_server_list.push_back("30.30.30.3:6379");
...
TEST_F(OptionsTest, CustomConfigFileAndOverrideFromCommandLine) {
    ...
    char argv_16[] = "--CASSANDRA.cassandra_use_ssl";
    ...
    vector<string> redis_server_list;
    redis_server_list.push_back("11.10.10.1:6379");
    redis_server_list.push_back("21.20.20.2:6379");
    redis_server_list.push_back("31.30.30.3:6379");
    TASK_UTIL_EXPECT_VECTOR_EQ(redis_server_list,
                               options_.redis_server_list());
    EXPECT_EQ(options_.redis_port(), 200);
