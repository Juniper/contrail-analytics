...
class OpServer(object):
    ...
    def __init__(self, args_str=' '.join(sys.argv[1:])):
        ...
        alarm_enable = os.getenv('ANALYTICS_ALARM_NODES', '')
        qe_enable = os.getenv('ANALYTICSDB_NODES', '')
        self._analytics_links = ['uves', 'uve-types']
        if qe_enable is not '':
            self._analytics_links.extend(['tables', 'queries'])
        if alarm_enable is not '':
            self._analytics_links.extend(['alarms', 'uve-stream', 'alarm-stream'])
        print "Bis: Getting qe_enable as:", qe_enable
        print "Bis: Getting alarm_enable as:", alarm_enable
