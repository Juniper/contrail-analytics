...
class AnalyticsUveTest(testtools.TestCase, fixtures.TestWithFixtures):
    ...
    def test_16_analytics_ssl_params_ssl_enable_true(self):
        ...
        generator_obj = self.useFixture(
            GeneratorFixture("contrail-vrouter-agent", collectors,
                             logging, vizd_obj.get_opserver_port(), analytics_ssl_params=analytics_ssl_params))
        assert generator_obj.verify_on_setup()
        generator_obj.send_vm_uve(vm_id='abcd',
                                  num_vm_ifs=5,
                                  msg_count=5)
    ...
    def test_17_analytics_ssl_params_ssl_insecure_enable_true(self):
        ...
        opserver using HTTPS.
        '''
        logging.info("%%% test_17_analytics_ssl_params_ssl_enable_true %%%")
    ...
    def test_20_analytics_ssl_params_client_ssl_not_enabled(self):
        ...
        vizd_obj = self.useFixture(
            AnalyticsFixture(logging, builddir, 0,
                             analytics_ssl_params=analytics_ssl_params))
        assert vizd_obj.verify_on_setup()
        collectors = [vizd_obj.get_collector()]
        generator_obj = self.useFixture(
            GeneratorFixture("contrail-vrouter-agent", collectors,
        ...
        generator_obj.set_analytics_ssl_config(None)
        assert not generator_obj.verify_on_setup()
        generator_obj.send_vm_uve(vm_id='abcd',
                                  num_vm_ifs=5,
                                  msg_count=5)
        assert generator_obj.verify_vm_uve(vm_id='abcd',
