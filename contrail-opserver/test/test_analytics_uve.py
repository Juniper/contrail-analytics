...
class AnalyticsUveTest(testtools.TestCase, fixtures.TestWithFixtures):
    ...
    #@unittest.skip('Skipping redis_ssl_redis_restart test')
    def test_16_redis_ssl_redis_restart(self):
        '''
        This test starts redis,vizd,opserver and qed in
        SSL enabled environment.
        Then it checks that the Generator list UVE is
        resynced after redis restart
        '''
        logging.info("%%% test_16_redis_ssl_redis_restart %%%")
        redis_ssl_params = {
            'keyfile': builddir+'/opserver/test/data/ssl/server-privkey.pem',
            'certfile': builddir+'/opserver/test/data/ssl/server.pem',
            'ca_cert': builddir+'/opserver/test/data/ssl/ca-cert.pem',
    ...
    # end test_16_redis_ssl_redis_restart
