...
class GeneratorFixture(fixtures.Fixture):
    ...
    def __init__(self, name, collectors, logger, opserver_port,
                 ...
                 hostname=socket.getfqdn("127.0.0.1").split('.')[0], inst = "0",
                 sandesh_config=None):
    ...
    def verify_on_setup(self):
        try:
            vg = VerificationGenerator(socket.getfqdn("127.0.0.1").split('.')[0], self._http_port, \
    ...
    def generate_intervn(self):
        self.send_vn_uve(socket.getfqdn("127.0.0.1").split('.')[0], 0, 2)
        time.sleep(1)
        self.send_vn_uve(socket.getfqdn("127.0.0.1").split('.')[0], 1, 3)
        time.sleep(1)
        self.send_vn_uve(socket.getfqdn("127.0.0.1").split('.')[0], 0, 3)
        time.sleep(1)

        self.vn_all_rows = {}
        self.vn_all_rows['whereclause'] = 'vn_stats.vrouter=' + socket.getfqdn("127.0.0.1").split('.')[0]
    ...
    def verify_vm_uve(self, vm_id, num_vm_ifs, msg_count, opserver_port=None):
        if opserver_port is not None:
            ...
            vns = VerificationOpsSrv(socket.getfqdn("127.0.0.1").split('.')[0], self._opserver_port)
