...
UserDefineSyslogParser::rx_config(const contrail_rapidjson::Document &jdoc, bool add_update) {
    ...
    LOG(DEBUG, opration << sb.GetString());
...
UserDefineSyslogParser::add_update_parser_config(std::string name, std::string parent_name,
    ...
    if (parser_d.HasMember("query_tags")) {
        ...
        if (parser_d["query_tags"].HasMember("tag_list")) {
            for (contrail_rapidjson::SizeType i = 0;
                 i < parser_d["query_tags"]["tag_list"].Size(); i++) {
                if (!parser_d["query_tags"]["tag_list"][i].IsString()) {
                    continue;
                }
                pParser->tag_list.insert(
                           parser_d["query_tags"]["tag_list"][i].GetString());
            }
        }
    }
    ...
    if (parser_d.HasMember("metrics")) {
        ...
        if (parser_d["metrics"].HasMember("metric_list")) {
            for (contrail_rapidjson::SizeType i = 0;
                 i < parser_d["metrics"]["metric_list"].Size(); i++) {
                if (!parser_d["metrics"]["metric_list"][i]["name"].IsString()) {
                    continue;
                }
                std::string name =
...
UserDefineSyslogParser::delete_parser_config(std::string name, std::string parent_name,
    ...
    syslog_parser_cfg_list_.erase(name);
}
...
UserDefineSyslogParser::add_update_rfc5424_config(std::string name,
    ...
    if (!rfc5424_d.HasMember("rfc5424_config")) {
        return;
    }
    prfc5424Info->hostname_as_tag = false;
    prfc5424Info->appname_as_tag = false;
    prfc5424Info->procid_as_tag = false;
    prfc5424Info->msgid_as_tag = false;
    if (rfc5424_d["rfc5424_config"].HasMember("hostname_as_tag")) {
        prfc5424Info->hostname_as_tag =
                         rfc5424_d["rfc5424_config"]["hostname_as_tag"].GetBool();
    }

    if (rfc5424_d["rfc5424_config"].HasMember("appname_as_tag")) {
        prfc5424Info->appname_as_tag =
                         rfc5424_d["rfc5424_config"]["appname_as_tag"].GetBool();
    }

    if (rfc5424_d["rfc5424_config"].HasMember("procid_as_tag")) {
        prfc5424Info->procid_as_tag =
                         rfc5424_d["rfc5424_config"]["procid_as_tag"].GetBool();
    }

    if (rfc5424_d["rfc5424_config"].HasMember("msgid_as_tag")) {
...
UserDefineSyslogParser::get_config(std::vector<LogParserConfigInfo> *config_info,
    ...
    for (SyslogParserList::iterator it = syslog_parser_cfg_list_.begin();
        ...
        pattern.set_pattern(gp_->get_pattern_by_name(it->first));
        for (uint32_t i = 0; i < it->second.sub_patterns.size(); i++) {
            vector<string> sep_list;
            boost::split(sep_list, it->second.sub_patterns[i],
                           boost::is_any_of(" "));
            std::string sub_pattern = gp_->get_pattern_by_name(sep_list[0]);
            sub_patterns.push_back(sep_list[0] + " " + sub_pattern);
        }
        pattern.set_sub_patterns(sub_patterns);
        parserInfo.set_pattern(pattern);
