...
RedisAsyncConnection::RedisAsyncConnection(EventManager *evm, const std::string & redis_ip,
        ...
        const bool redis_ssl_enable, const std::string & redis_keyfile,
...
bool RedisAsyncConnection::RAC_Connect(void) {
    ...
    if (redis_ssl_enable_) {
        int rc = redisSecureConnection(&context_->c, redis_ca_cert_.c_str(), redis_certfile_.c_str(), redis_keyfile_.c_str(), "sni");
        if (rc != REDIS_OK) {
