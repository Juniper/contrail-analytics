...
static void SendLogParserConfigInfoResponse(VizCollector *analytics, std::string context) {
    ...
    analytics->GetUserDefineSyslogConfig(&config_info, &preconfig, &preinstall_pattern);
    fcsr->set_config_info(config_info);
    fcsr->set_pre_config(preconfig);
    fcsr->set_preinstall_pattern(preinstall_pattern);
    fcsr->set_context(context);
    fcsr->Response();
}
